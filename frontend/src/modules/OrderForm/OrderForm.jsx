...
export const OrderForm = ({ event_id, orderId, onSave, onComplete }) => {
  ...
  useEffect(() => {
    if (orderDetails && event?.data) {
      ...
      // Set orderCategories from orderDetails.form.categories
      setOrderCategories(orderDetails.form.categories);

      // Initialize selectedProducts based on orderItems
      const newSelectedProducts = {};
      orderDetails.orderItems.forEach((item) => {
        newSelectedProducts[item.product.id] = {
      ...
      // Initialize groupedProducts
      const newGroupedProducts = {};
      event.data.form.categories.forEach((eventCategory) => {
        const category = orderDetails.form.categories.find(
          (c) => c.id === eventCategory.id
        );
        if (category) {
          newGroupedProducts[category.id] = category.products;
        }
  ...
  const handleSubmit = async (isTemp = false) => {
    ...
    try {
      ...
      console.error("Order save failed:", error);
      alert(
        "주문 수정에 실패했습니다. 모든 필수 필드를 입력했는지 확인해주세요."
      );
    }
  };
      ...
      <section className={styles.sectionWrap}>
        ...
        <div className={styles.sectionGroupWrap}>
          ...
          <div className={styles.sectionGroup}>
            <h4 className={styles.sectionLabel}>작성일자</h4>
            <p className={styles.sectionValue}>
              {new Date(orderDetails.created_at).toLocaleDateString("ko-KR")}
            </p>
          </div>
        </div>
                          ...
                          handleProductChange(category.id, "attributes", [
                            {
                              id: groupedProducts[category.id]?.find(
                                (p) =>
                                  p.id === parseInt(productInfo.productId, 10)
      ...
      <section className={styles.sectionWrap}>
        ...
        {payments.map((payment, index) => (
          <PaymentTable
            key={index}
            payment={payment}
            onPaymentChange={handlePaymentChange(index)}
            customerName={customerName}
            isEdit={true}
          />
        ))}
